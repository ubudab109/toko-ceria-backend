...
import { Info, List, MinusSquare, Package, PlusCircle, PlusSquare, Save, Tag } from "lucide-react";
...
export default function OrderForm({
    ...
    const [isDescriptionStatusShow, setIsDescriptionStatusShow] = useState<boolean>(false);
    ...
    const handleProductChange = (
        ...
        const updated = [...data.product_orders];
        if (selected) {
            updated[index].product_id = typeof selected.value === 'string' ? parseInt(selected.value) : selected.value
            setData('product_orders', updated)
        }
    }
    ...
    const subtotal = useMemo(() => {
        return data.product_orders
            ...
            .reduce((sum, prod) => {
                const selected = products.find((p: ProductI) => p.id === prod.product_id);
                return sum + (selected ? selected.price * (prod.quantity || 1) : 0);
            }, 0);
        ...
        <div className="w-full max-w-full p-6 sm:p-8 bg-white dark:bg-gray-800 shadow-2xl rounded-xl border border-gray-100 dark:border-gray-700 transition-all duration-300 mx-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-8" encType="multipart/form-data">
                ...
                <div className="space-y-6">
                    ...
                    {/* ORDER NUMBER */}
                    <FormField isRequired={true} id="order_number" label="Nomor Order" icon={Tag} errors={errors.order_number}>
                        <input
                            id="name"
                            type="text"
                        ...
                        <button
                            ...
                            onClick={async () => {
                                ...
                                const response = await axios.post(route('generate.order-number'));
                                setData('order_number', response.data.order_number);
                                toast.success(`Order Number berhasil dibuat: ${response.data.order_number}`);
                                setIsGeneratingOrderNumber(false);
                            }}
                        >
                            {
                                isGeneratingOrderNumber ? 'Tunggu Sebentar...' : 'Generate Order Number Otomatis'
                            }
                        </button>
                    </FormField>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        {/* CUSTOMER */}
                        <FormField isRequired={true} idHtmlFor="customer_id" id="customer_id" label="Customer" icon={List} errors={errors.customer_id}>
                            ...
                            <Select<OptionType, false>
                                ...
                                onChange={(selectedOption: SingleValue<OptionType>) => setData('customer_id', selectedOption?.value ?? null)}
                                value={customers
                                    .map((cus: CustomerI) => ({
                                        label: `${cus.fullname} - ${cus.email}`,
                                        value: cus.id,
                                    }))
                                    .find((opt) => opt.value === data.customer_id) || null}
                                styles={selectStyles}
                        ...
                        {/* CHECKOUT TYPE */}
                        <FormField isRequired={true} idHtmlFor="checkout_type" id="checkout_type" label="Tipe Checkout" icon={List} errors={errors.checkout_type}>
                            <Select<OptionType, false>
                                id="checkout_type"
                                options={PAYMENT_TYPE}
                                onChange={(selectedOption: SingleValue<OptionType>) => setData('checkout_type', selectedOption?.value ?? null)}
                                value={PAYMENT_TYPE
                                    .map((cus: OptionType) => ({
                                        label: `${cus.label}`,
                                        value: cus.value,
                                    }))
                                    .find((opt) => opt.value === data.checkout_type) || null}
                                styles={selectStyles}
                                classNamePrefix="react-select"
                        ...
                        {/* ORDER STATUS */}
                        <FormField isRequired={true} idHtmlFor="status" id="status" label="Status Order" icon={List} errors={errors.status}>
                            <Select<OptionType, false>
                                id="status"
                                options={statuses}
                                onChange={(selectedOption: SingleValue<OptionType>) => setData('status', selectedOption?.value ?? null)}
                                value={statuses
                                    .map((cus: OptionType) => ({
                                        label: `${cus.label}`,
                                        value: cus.value,
                                    }))
                                    .find((opt) => opt.value === data.status) || null}
                                styles={selectStyles}
                                classNamePrefix="react-select"
                                placeholder="Pilih Order Status"
                            />
                            <span onClick={() => setIsDescriptionStatusShow(true)} className="mt-2 cursor-pointer flex space-x-2 text-sm font-medium text-gray-700 dark:text-gray-300">
                                <Info className="mr-1" /> Klik disini untuk lihat deskripsi status
                            </span>
                        </FormField>
                    </div>

                    {/* NOTES */}
                    <FormField id="notes" label="Notes/Catatan Tambahan" icon={List} errors={errors.notes}>
                        <textarea
                            id="notes"
                            value={data.notes || ''}
                    ...
                    <div className="overflow-x-auto">
                        <div className="min-w-[950px] space-y-4">
                            ...
                            {errors.product_orders && (
                                <p className="mt-1 text-xs text-red-500 dark:text-red-400 font-medium">{errors.product_orders}</p>
                            )}
                            <div className="flex justify-between font-semibold text-sm text-gray-600 dark:text-gray-300 border-b pb-1">
                                <span className="w-[30%]">Product <span className="text-xs text-red-500 dark:text-red-400 font-medium">*</span></span>
                                <span className="w-[15%] text-center">Harga</span>
                                <span className="w-[15%] text-center">Quantity <span className="text-xs text-red-500 dark:text-red-400 font-medium">*</span></span>
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <div className="w-[30%]">
                                                <Select
                                                        ...
                                                        .filter(
                                                            (p) =>
                                                                !data.product_orders.some(
                                                                    (order, i) => order.product_id === p.id && i !== index && !order.is_deleted
                                                                )
                                                        )
                                                        .map((p) => ({
                                                    ...
                                                    value={
                                                        prod.product_id
                                                            ? (() => {
                                                                const selectedProduct = products.find((p) => p.id === prod.product_id);
                                                ...
                                                {/* IF STOCK IS EMPTY */}
                                                {prod.product_id &&
                                                    products.find((p: ProductI) => p.id === prod.product_id)?.stock === 0 ? (
                                            ...
                                            <div className="w-[10%] flex justify-center gap-2">
                                                ...
                                                {index === data.product_orders.length - 1 && (
                                                    <button
                                                        type="button"
                                                        onClick={handleAdd}
